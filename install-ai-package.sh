...
STARTUP_ROUTING_START="<!-- ai-starter-kit:startup-routing:start -->"
STARTUP_ROUTING_END="<!-- ai-starter-kit:startup-routing:end -->"

# Pulls the marked block out of a file (exclusive of the marker lines
# themselves). Used to lift the "Callable skills & Autonomous Invocation"
# section out of STARTUP.md so it can be merged into every agent's own
# main file — STARTUP.md itself is copied into the destination's .ai/ but
# nothing reads it there automatically, so without this its skill-routing
# directive would never reach an agent that lacks native skill discovery.
...
combine_context_and_rules() {
  ...
  if [[ -n "${4:-}" && -f "$4" ]]; then
    local startup_block
    startup_block="$(extract_marked_block "$4" "$STARTUP_ROUTING_START" "$STARTUP_ROUTING_END")"
    if [[ -n "$startup_block" ]]; then
      local startup_tmp
      startup_tmp="$(mktemp)"
      printf '%s\n' "$startup_block" > "$startup_tmp"
      append_section_file "$tmp_file" "Callable Skills & Autonomous Invocation" "$startup_tmp"
      rm -f "$startup_tmp"
    fi
  fi
...
generate_claude() {
  local dest="$1"
  combine_context_and_rules "$dest/CLAUDE.md" "$dest/.ai/context/README.md" "$dest/.ai/rules" "$dest/.ai/STARTUP.md"
  ...
  cleanup_stale_openspec_static "$dest" "claude"
...
generate_cursor() {
  local dest="$1"
  combine_context_and_rules "$dest/.cursorrules" "$dest/.ai/context/README.md" "$dest/.ai/rules" "$dest/.ai/STARTUP.md"
  ...
  cleanup_stale_openspec_static "$dest" "cursor"
  local skill_files=() skill_file
  while IFS= read -r skill_file; do skill_files+=("$skill_file"); done \
    < <(list_non_readme_markdown "$dest/.ai/context/skills")
  local total="${#skill_files[@]}" step=0
...
generate_windsurf() {
  local dest="$1"
  combine_context_and_rules "$dest/.windsurfrules" "$dest/.ai/context/README.md" "$dest/.ai/rules" "$dest/.ai/STARTUP.md"
  ...
  cleanup_stale_openspec_static "$dest" "windsurf"
  local skill_files=() skill_file
  while IFS= read -r skill_file; do skill_files+=("$skill_file"); done \
    < <(list_non_readme_markdown "$dest/.ai/context/skills")
  local total="${#skill_files[@]}" step=0
...
generate_jetbrains() {
  local dest="$1"
  combine_context_and_rules "$dest/.aiassistant/rules/project.md" "$dest/.ai/context/README.md" "$dest/.ai/rules" "$dest/.ai/STARTUP.md"
...
do_check() {
  ...
  case "$workflow" in
    openspec)
      ...
      # This kit's own static opsx-* files (from .ai/prompts and .ai/skills,
      # copied flat by generate_<agent>()) and the real OpenSpec CLI's own
      # generated files can both exist at once for the same agent —
      # do_openspec cleans this up going forward, but an install from before
      # that cleanup existed, or a manual `openspec init` run outside this
      # script, can still have both. Confirmed real CLI output locations
      # (OpenSpec CLI 1.13.0): .claude/commands/opsx/*, .cursor/commands/opsx/*
      # — and, for windsurf, .devin/skills/openspec-*/SKILL.md +
      # .devin/workflows/opsx-*.md. The CLI's `--tools windsurf` value now
      # only exists as a deprecated alias for `devin` (its own --help says
      # "windsurf (now devin)"): it renamed that integration to "Devin Desktop
      # (formerly Windsurf)" and moved its OpenSpec output out of `.windsurf/`
      # entirely, while this kit's own (non-OpenSpec) Windsurf conventions
      # — .windsurfrules, .windsurf/rules/*, .windsurf/workflows/commit-and-push.md
      # etc. — are untouched by that rename and still live under `.windsurf/`.
      local dup_agent dup_static dup_native
      for dup_agent in claude cursor windsurf; do
        case "$dup_agent" in
          claude) dup_static="$dest/.claude/commands/opsx-*.prompt.md"; dup_native="$dest/.claude/commands/opsx" ;;
          cursor) dup_static="$dest/.cursor/commands/opsx-*.prompt.md"; dup_native="$dest/.cursor/commands/opsx" ;;
          windsurf) dup_static="$dest/.windsurf/workflows/openspec-*.md"; dup_native="$dest/.devin/skills" ;;
        esac
        if [[ -d "$dup_native" ]] && compgen -G "$dup_static" >/dev/null 2>&1; then
          warn "[$dup_agent] Both this kit's static $(basename "$dup_static") files and the OpenSpec CLI's own" \
               "${dup_native#"$dest"/} output are present — run --openspec $dup_agent $dest again to remove" \
               "the stale static copies, or delete them by hand."
          warnings=$((warnings + 1))
        fi
      done
      if [[ -d "$dest/.devin" && -d "$dest/.windsurf" ]]; then
        ok "[windsurf] OpenSpec integration present at .devin/ (the CLI renamed this integration to" \
           "\"Devin Desktop (formerly Windsurf)\" — this kit's own non-OpenSpec Windsurf files stay under .windsurf/)"
      fi
      ;;
    speckit)
...
prune_workflow() {
  ...
  case "$workflow" in
    ...
    speckit|none)
      ...
      # Also remove the *real* OpenSpec CLI's own generated output, if
      # --openspec was ever run before switching away from this workflow —
      # prune_workflow previously only cleaned this kit's static snapshot,
      # leaving these behind forever. `.claude/commands/opsx/` and
      # `.cursor/commands/opsx/` hold only OpenSpec content (safe to remove
      # outright); `.devin/` is entirely OpenSpec's own output for this tool
      # (see openspec_tool_for()), so it's removed wholesale too.
      for dir in "$dest"/.claude/commands/opsx \
                 "$dest"/.cursor/commands/opsx \
                 "$dest"/.devin; do
        [[ -d "$dir" ]] || continue
        rm -rf "$dir"
        removed=1
...
# `openspec init --help`, CLI 1.13.0). Left blank = no confirmed mapping.
#
# windsurf -> devin: the CLI's own --help says "Also accepted: windsurf (now
# devin)" — `windsurf` is a deprecated compatibility alias, `devin` is the
# real, current --tools value (confirmed: both produce identical output, a
# tool called "Devin Desktop (formerly Windsurf)" whose OpenSpec files land
# under `.devin/`, not `.windsurf/`). Use the non-deprecated value in case a
# future CLI release drops the alias. This only affects OpenSpec's own
# generated output for this tool — this kit's own (non-OpenSpec) Windsurf
# conventions (.windsurfrules, .windsurf/rules/*, .windsurf/workflows/*) are
# a separate, unconfirmed-whether-renamed convention and are left as-is; see
# cleanup_stale_openspec_static() below.
openspec_tool_for() {
  case "$1" in
    copilot) printf 'github-copilot' ;;
    claude) printf 'claude' ;;
    cursor) printf 'cursor' ;;
    windsurf) printf 'devin' ;;
    jetbrains) printf '' ;;
    *) printf '' ;;
  esac
}
...
# Once the real OpenSpec CLI has run for an agent, its own generated files
# become the authoritative copy — but they land at a different path than
# this kit's static/frozen ones (confirmed against CLI 1.13.0):
#   claude:   .claude/commands/opsx/*        vs. this kit's .claude/commands/opsx-*.prompt.md
#   cursor:   .cursor/commands/opsx/*        vs. this kit's .cursor/commands/opsx-*.prompt.md
#   windsurf: .devin/skills/openspec-*/SKILL.md + .devin/workflows/opsx-*.md
#             vs. this kit's .windsurf/workflows/openspec-*.md
# (windsurf's real destination isn't even under .windsurf/ — the CLI renders
# that integration as "Devin Desktop (formerly Windsurf)" and writes
# everything OpenSpec-related to .devin/ instead; see openspec_tool_for()
# above. This kit's own non-OpenSpec Windsurf files — .windsurfrules,
# .windsurf/rules/*, .windsurf/workflows/commit-and-push.md /
# neo4j-architecture-graph.md — are a separate convention, untouched by this
# and left alone here.)
# `openspec init --force` never touches these paths (it does clean up its
# OWN previous naming conventions, e.g. old .windsurf/workflows/openspec-*.md
# from before the .devin/ move, but has no way to know about this kit's
# separately-shipped static snapshot), so without this cleanup both sets can
# linger side by side indefinitely — two diverging command sets for the same
# workflow, with nothing telling the user which one is current. Copilot is
# deliberately not included: this kit has no confirmed OpenSpec CLI output
# location for it to compare against, so its static
# .github/prompts/opsx-*.prompt.md is left alone rather than guessed away.
#
# Self-guarding on the real CLI's own marker directory (rather than just
# deleting unconditionally) is what makes this safe to call from every
# generate_<agent>() below, not only right after `--openspec` runs: each
# generate_<agent>() unconditionally re-copies this kit's static snapshot
# from .ai/ on every --agents/--sync/--update, which would otherwise
# resurrect the very files --openspec just removed on the next sync. Before
# `--openspec` has ever run, the marker directory doesn't exist yet, so this
# is a no-op and the static snapshot is correctly left as the working
# fallback.
cleanup_stale_openspec_static() {
  local dest="$1" agent="$2"
  local removed=0 f native_marker
  case "$agent" in
    claude) native_marker="$dest/.claude/commands/opsx" ;;
    cursor) native_marker="$dest/.cursor/commands/opsx" ;;
    windsurf) native_marker="$dest/.devin/skills" ;;
    *) return 0 ;;
  esac
  [[ -d "$native_marker" ]] || return 0

  case "$agent" in
    claude)
      for f in "$dest"/.claude/commands/opsx-*.prompt.md; do
        [[ -f "$f" ]] || continue
        rm -f "$f"
        removed=1
      done
      ;;
    cursor)
      for f in "$dest"/.cursor/commands/opsx-*.prompt.md; do
        [[ -f "$f" ]] || continue
        rm -f "$f"
        removed=1
      done
      ;;
    windsurf)
      for f in "$dest"/.windsurf/workflows/openspec-*.md; do
        [[ -f "$f" ]] || continue
        rm -f "$f"
        removed=1
      done
  ...
  if [[ "$agent" == "windsurf" ]]; then
    ok "[$agent] Removed this kit's static openspec-*.md workflow files — the OpenSpec CLI now" \
       "manages this integration (branded \"Devin Desktop, formerly Windsurf\") under .devin/ instead."
  else
    ok "[$agent] Removed this kit's static opsx-* command files — the OpenSpec CLI's own copy (already installed) is authoritative."
  fi
}
...
do_openspec() {
  ...
  local agent agents=() tools=() mapped_agents=()
  while IFS= read -r agent; do
    [[ -n "$agent" ]] && agents+=("$agent")
  done < <(tr ',' '\n' <<< "$agents_csv")
  ...
  if (cd "$dest" && $runner init --tools "$tools_csv" --force); then
    ...
    for agent in "${mapped_agents[@]}"; do
      cleanup_stale_openspec_static "$dest" "$agent"
    done
  else
